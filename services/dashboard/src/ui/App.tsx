...
type TeamHealth = { status?: string; service?: string; version?: string; error?: string };
type StatusResponse = { fraud: TeamHealth; recs: TeamHealth; forecast: TeamHealth };
...
export default function App() {
  ...
  const [payload, setPayload] = useState<string>('{"userId": 42, "k": 5}');
  const [team, setTeam] = useState<"fraud"|"recs"|"forecast">("recs");
  const [predictOut, setPredictOut] = useState<any>(null);
  ...
  async function sendTest() {
    ...
    try {
      ...
      try { setPredictOut(JSON.parse(text)); } catch { setPredictOut(text); }
    } catch (e:any) {
  ...
  useEffect(() => {
    refresh();
    const t = setInterval(refresh, 5000); // live polling
  ...
  return (
    <div style={{ fontFamily: "system-ui, -apple-system, Segoe UI, Roboto, sans-serif", padding: 24, maxWidth: 1100, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", gap: 12, marginTop: 12 }}>
        <button onClick={refresh} style={{ padding: "10px 14px", borderRadius: 12, border: "1px solid #ddd", background: "white" }}>
          Refresh
        </button>
      ...
      <div style={{ marginTop: 18, border: "1px solid #eee", borderRadius: 16, overflow: "hidden" }}>
            ...
            {rows.map((r) => {
              ...
              return (
                <tr key={r.team} style={{ borderTop: "1px solid #eee" }}>
                  ...
                  <td style={{ padding: 12 }}><span style={badge(ok)}>{ok ? "Healthy" : "Down"}</span></td>
                  <td style={{ padding: 12 }}>{r.data?.service || "-"}</td>
                  <td style={{ padding: 12 }}>{r.data?.version || "-"}</td>
                  <td style={{ padding: 12, color: ok ? "#888" : "crimson" }}>{r.data?.error || "-"}</td>
                </tr>
              );
            })}
          </tbody>
      ...
      <div style={{ marginTop: 18, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
        <div style={{ border: "1px solid #eee", borderRadius: 16, padding: 16 }}>
          ...
          <div style={{ display: "flex", gap: 10, marginBottom: 10 }}>
            ...
            <select value={team} onChange={(e) => setTeam(e.target.value as any)} style={{ padding: 8, borderRadius: 10 }}>
              <option value="fraud">fraud</option>
              <option value="recs">recs</option>
              <option value="forecast">forecast</option>
            </select>
            <button onClick={sendTest} style={{ padding: "10px 14px", borderRadius: 12, border: "1px solid #ddd", background: "white" }}>
              Send
            </button>
          </div>
          <textarea
            ...
            rows={10}
            style={{ width: "100%", padding: 12, borderRadius: 12, border: "1px solid #ddd", fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace" }}
          />
        </div>

        <div style={{ border: "1px solid #eee", borderRadius: 16, padding: 16 }}>
          <h2 style={{ marginTop: 0 }}>Response</h2>
          <pre style={{ background: "#fafafa", border: "1px solid #eee", borderRadius: 12, padding: 12, overflow: "auto", height: 300 }}>
